...
cursor.execute("""
    SELECT 
        ...
        checkedLee
    FROM movies
    WHERE criterion IS NOT NULL OR owned = 'yes'
...
for row in data:
    ...
    rating = f"Metacritic: {row[5]}%" if row[5] is not None else 'n/a'

    formatted_row = [
        Paragraph(f"{title}{info}{rating}", styles['Small']),
